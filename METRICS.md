...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **M01** | `core1_boot_latency` | Time elapsed from Core 0 FIFO write to Core 1 log | microseconds | < 500 | **45** | SIO FIFO Timer Capture |
...
| **M06** | `fft_execution_time` | Computation latency for 512-point fixed-point FFT | microseconds | < 500 | **220** | DWT Cycle Counter |
| **M07** | `folding_sample_latency`| Execution time to fold a single sample on Core 1 | CPU cycles | < 10 | **3** | DWT Cycle Counter |
...
| **M10** | `net_packet_drops` | Cumulative count of dropped VITA-49 packet sequence gaps | count | < 100 | **0** | UDP sequence check counter |
